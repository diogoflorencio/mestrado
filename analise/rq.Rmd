...
### Relação tamanho da notícia e popularidade
...
len_pop = data.frame(rbind(pearson_ic, spearman_ic))

len_pop$coef = c("pearson", "spearman")

len_pop %>% 
  ggplot(aes(x = coef, ymin = X2.5., ymax = X97.5.)) + 
  geom_errorbar(width = .2, color="blue") +
...
### Relação legibilidade da notícia e popularidade

```{r}
read_metaData <- merge(x = readabilityMetrics, y = metaData, all = TRUE) %>% filter(!is.na(jornal))

pearson_ic <- read_metaData %>% 
  bootstrap(cor(coleman_liau, number_comments, method = "pearson"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))

spearman_ic <- read_metaData %>% 
  bootstrap(cor(coleman_liau, number_comments, method = "spearman"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))

coleman_pop = data.frame(rbind(pearson_ic, spearman_ic))
coleman_pop$coef = c("pearson", "spearman")

pearson_ic <- read_metaData %>% 
  bootstrap(cor(smog_, number_comments, method = "pearson"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))

spearman_ic <- read_metaData %>% 
  bootstrap(cor(smog_, number_comments, method = "spearman"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))

smog_pop = data.frame(rbind(pearson_ic, spearman_ic))
smog_pop$coef = c("pearson", "spearman")

pearson_ic <- read_metaData %>% 
  bootstrap(cor(lix, number_comments, method = "pearson"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))

spearman_ic <- read_metaData %>% 
  bootstrap(cor(lix, number_comments, method = "spearman"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))

lix_pop = data.frame(rbind(pearson_ic, spearman_ic))
lix_pop$coef = c("pearson", "spearman")

pearson_ic <- read_metaData %>% 
  bootstrap(cor(dale_chall, number_comments, method = "pearson"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))

spearman_ic <- read_metaData %>% 
  bootstrap(cor(dale_chall, number_comments, method = "spearman"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))

dale_chall_pop = data.frame(rbind(pearson_ic, spearman_ic))
dale_chall_pop$coef = c("pearson", "spearman")


read_pop_ci <-bind_rows(
    "coleman index" = coleman_pop,
    "smog index" = smog_pop, 
    "lix index" = lix_pop, 
    "dale chall score" = dale_chall_pop,
    .id = "readability_metric")

read_pop_ci %>% 
  ggplot(aes(x = coef, ymin = X2.5., ymax = X97.5., color = coef)) + 
  geom_errorbar(width = .2) +
  labs(title = "Relação legibilidade da notícia e popularidade",
       subtitle = "Correlação entre a legibilidade da notícia e sua popularidade",
       x = "",
       y = "") +
  facet_wrap( ~readability_metric) +
...
subj_ci %>% 
  ...
  geom_errorbar(width = .2) +
  labs(title = "Percentual de subjetividade por jornal",
       subtitle = "Estimativa de subjetividade considerando todas as notícias em um ano de coleta",
       x = "", 
       y = "") +
  theme(legend.position = "none")
...
spearman_ic <- readabilityMetrics %>% 
  bootstrap(cor(bias_news, coleman_liau_index, method = "spearman"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))
cor_coleman = data.frame(rbind(pearson_ic, spearman_ic))
cor_coleman$coef = c("pearson", "spearman")
pearson_ic <- readabilityMetrics %>% 
...
spearman_ic <- readabilityMetrics %>% 
  bootstrap(cor(bias_news, smog_index, method = "spearman"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))
cor_smog = data.frame(rbind(pearson_ic, spearman_ic))
cor_smog$coef = c("pearson", "spearman")
pearson_ic <- readabilityMetrics %>% 
...
spearman_ic <- readabilityMetrics %>% 
  bootstrap(cor(bias_news, lix_index, method = "spearman"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))
cor_lix = data.frame(rbind(pearson_ic, spearman_ic))
cor_lix$coef = c("pearson", "spearman")
pearson_ic <- readabilityMetrics %>% 
...
spearman_ic <- readabilityMetrics %>% 
  bootstrap(cor(bias_news, dale_chall_score, method = "spearman"), 
            R = 2000) %>% 
    CI.percentile(probs = c(.025, .975))
cor_dale_chall = data.frame(rbind(pearson_ic, spearman_ic))
cor_dale_chall$coef = c("pearson", "spearman")
read_ci <-bind_rows(
    "coleman index" = cor_coleman,
    "smog index" = cor_smog, 
    "lix index" = cor_lix, 
    "dale chall score" = cor_dale_chall,
    .id = "readability_metric")
read_ci %>% 
  ggplot(aes(x = coef, ymin = X2.5., ymax = X97.5., color = coef)) + 
  geom_errorbar(width = .2) +
  labs(title = "Relação subjetividade e legibilidade",
...
subj_len = data.frame(rbind(pearson_ic, spearman_ic))

subj_len$coef = c("pearson", "spearman")

subj_len %>% 
  ggplot(aes(x = coef, ymin = X2.5., ymax = X97.5., color = coef)) + 
  geom_errorbar(width = .2) +
  labs(title = "Relação subjetividade e tamanho da notícia",
       subtitle = "Estimativa de correlação entre subjetividade  e tamanho da notícia",
       x = "", 
       y = "") +
  theme(legend.position = "none")
```
