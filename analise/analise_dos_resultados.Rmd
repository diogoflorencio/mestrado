...
```{r}
biasComments <- read_csv(here::here("data/bias_comments.csv")) %>% filter(arg >= 0, sen >= 0, val >= 0, pre >= 0, mod >= 0)
biasComments[["url"]] <- biasComments[["url"]][0]
...
biasNews <- news %>% 
  mutate(bias_news = (1 - (arg + sen + val + pre + mod) / 4) * 100) %>% 
