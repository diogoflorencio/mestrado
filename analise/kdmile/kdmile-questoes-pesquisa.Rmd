...
a <- highchart() %>% 
  ...
  hc_yAxis(#title = list(text = "percentage of bias"),
           labels = list(format = "{value}%"), max = 83) %>% 
  hc_tooltip(pointFormat = "{point.y}%") %>% 
  #hc_add_series(name = "diversos" , data = (bias_section %>% filter(section == "diversos"))$bias) %>%  
  hc_add_series(name = "Carta Capital" , data = (bias_section %>% filter(jornal == "Carta Capital"))$bias) %>%  
  hc_add_series(name = "El País" , data = (bias_section %>% filter(jornal == "El País"))$bias) %>% 
  hc_add_series(name = "Estadão" , data = (bias_section %>% filter(jornal == "Estadão"))$bias) %>% 
  hc_add_series(name = "Folha de SP" , data = (bias_section %>% filter(jornal == "Folha de SP"))$bias) %>% 
  hc_add_series(name = "O Antagonista" , data = (bias_section %>% filter(jornal == "O Antagonista"))$bias) %>% 
  hc_add_series(name = "O Globo" , data = (bias_section %>% filter(jornal == "O Globo"))$bias) %>% 
...
max.temp <- c(756.7, 819.2, 374.4, 543.8, 75.44, 506.2, 373.5)
barplot(max.temp,
main = "",
xlab = "",
ylab = "Media Outlets",
names.arg = c("Carta Capital", "El País", "Estadão", "Folha de SP", "O Antagonista", "O Globo", "Veja"),
border="red",
col="blue",
density=10
)

ggsave("fig9.pdf", width = 12, height = 4)
```

```{r}
df <- data.frame(dose=c("Carta Capital", "El País", "Estadão", "Folha de SP", "O Antagonista", "O Globo", "Veja"),
                len=c(756.7, 819.2, 374.4, 543.8, 75.44, 506.2, 373.5))
...
labs(#title = "Relação entre subjetividade e popularidade",
  ...
  theme(legend.position="none", 
        axis.text=element_text(size=20),
        plot.title = element_text(size=22)) + coord_flip()


ggsave("fig9.pdf", width = 8, height = 6)
